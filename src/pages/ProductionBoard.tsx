...
import { productionRounds, milkLots } from '../data/mockData';
...
const statusFlow = ['in_progress', 'pressed', 'cutting', 'frozen', 'packing', 'completed'];
      ...
      {statusFlow.map((step, i) => (
        <div key={step} className="flex items-center">
          ...
          {i < statusFlow.length - 1 && (
            <div className={`w-3 h-0.5 ${i < currentIndex ? statusColors[step] : 'bg-slate-200'}`} />
          )}
        </div>
      ))}
    </div>
...
export default function ProductionBoard() {
  ...
  // Group by shift for visual organization
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {activeMilkLot?.lotCode} • {filteredRounds.length} rounds shown
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div>
            ...
            >
              ...
              <option value="D">D (Diced)</option>
              <option value="C/S">C/S (Cut & Sliced)</option>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-100">
                      <th className="px-4 py-2 text-left font-medium text-slate-500 text-xs uppercase tracking-wide w-36">
                        Batch ID
                      </th>
                      <th className="px-4 py-2 text-left font-medium text-slate-500 text-xs uppercase tracking-wide w-20">
                        Type
                      </th>
                      <th className="px-4 py-2 text-left font-medium text-slate-500 text-xs uppercase tracking-wide">
                        Pipeline
                      </th>
                      <th className="px-4 py-2 text-left font-medium text-slate-500 text-xs uppercase tracking-wide w-24">
                        Status
                      </th>
                      <th className="px-4 py-2 text-left font-medium text-slate-500 text-xs uppercase tracking-wide w-20">
                        Input
                      ...
                      <tr key={round.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          {round.intermediateBalance !== undefined && round.intermediateBalance > 0 ? (
                            <span className="text-amber-700 font-medium">{round.intermediateBalance} kg</span>
                          ) : (
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-xs text-slate-500">{round.team.join(', ')}</span>
                          </div>
                          {round.cutBy && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-white rounded-xl border border-slate-200 p-3 text-center">
          ...
          <p className="text-xl font-bold text-slate-900 mt-1">
            {filteredRounds.reduce((s, r) => s + r.actualInput, 0)} L
          </p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-3 text-center">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-3 text-center">
          <p className="text-xs text-slate-500 uppercase tracking-wide">Intermediate</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-3 text-center">
          ...
          <p className="text-xl font-bold text-indigo-600 mt-1">
            {(() => {
              const withOutput = filteredRounds.filter((r) => r.outputWeight > 0);
