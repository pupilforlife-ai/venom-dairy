...
export default function Handover() {
  const { finishedStock, updateFinishedStock } = useApp();
  ...
  const selectAllAwaiting = () => {
    setSelectedIds(awaiting.map(item => item.id));
    showToast('success', `${awaiting.length} item(s) selected for handover`);
  };

  const confirmHandover = (id: string) => {
    updateFinishedStock(id, { status: 'handed_over' });
    setSelectedIds(current => current.filter(selectedId => selectedId !== id));
    showToast('success', 'Handover confirmed');
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <button onClick={selectAllAwaiting} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium bg-emerald-600 text-white hover:bg-emerald-700 transition-colors">
              ...
              {awaiting.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3">
                    <input type="checkbox" checked={selectedIds.includes(item.id)} onChange={() => toggleSelected(item.id)} className="w-4 h-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500" />
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => confirmHandover(item.id)} className="flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-700 rounded text-xs font-medium hover:bg-emerald-100 transition-colors">
                      Confirm
